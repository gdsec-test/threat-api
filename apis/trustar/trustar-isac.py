...
def initAppSecHandler() -> logging.StreamHandler:
    ...
    cloud_instance_id = 'arn:aws:lambda:{}:{}:function:{}'.format(AWS_REGION, MODULE_NAME, cloud_account_id)
    formatter = AppSecFormatter('threat-api', 'prod', [ 'threat-intel' ], cloud_account_id, cloud_instance_id)
...
def retrieveSecrets() -> Dict[str, str]:
    ...
    if secret is None:
        log.error('Failed to get the {} secrets but did not throw an exception'.format(SECRETS_MANAGER_ID))
        return None
...
def convertIndicator(ioc:str, indicators: Generator[trustar.Indicator, None, None]) -> List[Dict[str, Any]]:
...
def convertTimestamp(epoch: int) -> str:
    ...
    return datetime.datetime.utcfromtimestamp(epoch / 1000).strftime('%Y-%m-%dT%H:%M:%SZ')
...
def convertToCsv(ioc_dict: Dict[str, List[Dict[str, Any]]]) -> str:
    ...
    for ioc, indicators in ioc_dict.items():
        for indicator in indicators:
            writer.writerow([ioc, convertTimestamp(indicator['firstSeen']), convertTimestamp(indicator['lastSeen'])])
...
def process(job_request: Dict[str, str]) -> Dict[str, str]:
    ...
    if ioc_type == 'DOMAIN':
        ...
        ioc_dict = {ioc:convertIndicator(ioc, lookupDomainOrUrl(ts, ioc)) for ioc in ioc_list}
    elif ioc_type == 'MD5':
        ...
        ioc_dict = {ioc:convertIndicator(ioc, lookupSha256(ts, ioc)) for ioc in ioc_list}
    elif ioc_type == 'IP':
        ...
        ioc_dict = {ioc:convertIndicator(ioc, lookupEmailAddress(ts, ioc)) for ioc in ioc_list}
    elif ioc_type == 'CVE':
    ...
    metadata = '1 response found' if response_count == 1 else '{} responses found'.format(response_count),
    response_message = {
