...
EXCLUDE_FIELDS = [
    'origin', 'process', 'log', 'ecs'
]

class AppSecFormatter(ecs_logging.StdlibFormatter):
    def __init__(self, service_name, environment, tags, cloud_account_id, cloud_instance_id, cloud_instance_name=None, default_event=None):
...
class AppSecLogger(logging.Logger):
    def event(self, current_event):
        ...
        """
        appsecformatters = list(map(lambda x: x.formatter, filter(lambda x: isinstance(x.formatter, AppSecFormatter), self.handlers)))
