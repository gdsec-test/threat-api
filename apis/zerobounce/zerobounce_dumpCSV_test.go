...
func TestDumpCSV(t *testing.T) {
	...
	Convey("dumpCSV", t, func() {
		...
		Convey("should dump proper CSV output for EMAIL", func() {
			...
			expectedCSV := "Email Address,Status,Sub Status,Free Email,Did You Mean?,Account,Domain,Domain Age Days,SMTP Provider,MX Found?,MX Record,Processed At\n"

			reports := map[string]*zb.ZeroBounceReport{
			...
			actualCSV := DumpCSV(reports, ZeroBounceMetaData)
			Println(actualCSV)
